...
class MessagesController extends Controller
{
    ...
    public function getContacts(Request $request)
    {
        ...
        $users = Message::join('users',  function ($join) {
            ...
            ->where('ch_messages.workspace_id', '=', session()->get('workspace_id'))
    ...
    {
        $favoritesList = null;
        $favorites = Favorite::where('user_id', Auth::user()->id)->where('workspace_id', session()->get('workspace_id'));
    ...
    public function search(Request $request)
    {
        ...
        $workspace_users = $workspace->users()->pluck('users.id')->toArray();

        $records = User::whereIn('id', $workspace_users)
            ->where('id', '!=', Auth::user()->id)
            ->where(DB::raw('CONCAT_WS(" ", first_name, last_name)'), 'LIKE', "%{$input}%")
            ->paginate($request->per_page ?? $this->perPage);
