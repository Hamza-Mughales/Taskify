...
class CheckMaxCreate
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        if (Auth::check()) {
            ...
            // dd($subscriptionFeatures);
            $currentRoute = $request->route()->getName();

            // $projects =

            if (str_contains($currentRoute, '.create') || str_contains($currentRoute, '.duplicate')) {
                ...
                switch ($resource) {
                    ...
                    case 'users':
                        if (
                            $users >= $subscriptionFeatures['max_team_members']
                        ) {
                        ...
                        break;
                    case 'workspaces':
                        if ($workspaces >= $subscriptionFeatures['max_workspaces']) {
