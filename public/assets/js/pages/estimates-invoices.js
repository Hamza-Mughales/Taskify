...
function actionsFormatter(value, row, index) {
    ...
    return [
        '<a href="/' + urlPrefix + '/estimates-invoices/edit/' + row.id + '" title=' + label_update + ' class="card-link"><i class="bx bx-edit mx-1"></i></a>' +
        ...
        '</a>' +
        '<a href="/' + urlPrefix + '/estimates-invoices/pdf/' + row.id + '" title="PDF" target="_blank">' +
...
function idFormatter(value, row, index) {
    ...
    return [
        '<a href=  " ' + routePrefix + '/estimates-invoices/view/' + row.id + '" target="_blank">' + idPrefix + row.id + '</a>'
    ];
}
...
$('#client_id').on('change', function (e) {
    ...
    if (client_id != '') {
        $.ajax({
            url: '/' + urlPrefix + '/clients/get/' + client_id,
            type: 'get',
            headers: {
...
$('#item_id').on('change', function (e) {
    ...
    if (item_id != '') {
        $.ajax({
            type: 'get',
            url: '/' + urlPrefix + '/items/get/' + item_id,
            headers: {
                'X-CSRF-TOKEN': $('input[name="_token"]').attr('value') // Replace with your method of getting the CSRF token
            },
            success: function (result) {
...
function update_amount(itemIndex, isUpdateTotals = 1) {
    ...
    if (tax_id != '') {
        $.ajax({
            url: '/' + urlPrefix + '/taxes/get/' + tax_id,
            headers: {
